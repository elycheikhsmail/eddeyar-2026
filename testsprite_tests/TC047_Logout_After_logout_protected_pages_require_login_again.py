...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to the login page at /ar/p/users/connexion so the login form inputs become available.
        await page.goto("http://localhost:3000/ar/p/users/connexion", wait_until="commit", timeout=10000)
        
        # -> Fill the phone field with a test phone number and the password field, then submit the login form.
        ...
        # -> Navigate to /ar/my/list so the account page can be accessed (or redirect to login if not authenticated).
        await page.goto("http://localhost:3000/ar/my/list", wait_until="commit", timeout=10000)
        
        # -> Log in by filling the phone and password fields and submitting the form so the session can be established (then will proceed to logout).
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/main/div/form/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('36000000')
        ...
        # -> Fill the phone and password fields and submit the login form to attempt establishing a session.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/main/div/form/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('36000000')
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=Please log in').first).to_be_visible(timeout=3000)
