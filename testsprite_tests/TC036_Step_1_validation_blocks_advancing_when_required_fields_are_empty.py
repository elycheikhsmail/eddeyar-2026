...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /ar/p/users/connexion (the login page)
        await page.goto("http://localhost:3000/ar/p/users/connexion", wait_until="commit", timeout=10000)
        
        # -> Fill the phone and password fields and submit the login form to proceed to the site (then navigate to /ar/my/add).
        ...
        # -> Navigate to /ar/my/add to reach the ad creation wizard (to then attempt continuing with required fields left empty).
        await page.goto("http://localhost:3000/ar/my/add", wait_until="commit", timeout=10000)
        
        # --> Assertions to verify final state
        frame = context.pages[-1]
        await expect(frame.locator('text=هذا الحقل مطلوب').first).to_be_visible(timeout=3000)
        assert '/ar/my/add' in frame.url
